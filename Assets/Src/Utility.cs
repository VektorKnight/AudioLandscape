...
public static class Utility {
    ...
    public static Mesh GenerateGrid(int width, int height, bool horizontal) {
        ...
        // The winding of the vertices is arbitrary as long as you do it in a clockwise fashion.
        // Winding order determines which direction the triangle faces.
        // Unity and most other renderers will use clockwise winding to differentiate front and back faces.
        for (var i = 0; i < triangles.Length; i += 6) {
            // Vertex index.
            var vi = (i / 6);

            // Shift the index forward by the current row index.
            // We need to do this to prevent triangles incorrectly referencing vertices on the edges.
            vi += vi / (height);
    ...
    /// Resolution just indicates how many points we sample along the gradient.
    /// Ultimately, you may have to play with this number to determine the optimal resolution.
    /// As a general rule, the more complex the gradient, the higher the sampling resolution should be.
    /// </summary>
